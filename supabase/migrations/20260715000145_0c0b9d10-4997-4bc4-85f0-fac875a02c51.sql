...
BEGIN
  ...
  IF NEW.purchase_invoice_id IS NOT NULL THEN
    SELECT pi.total_amount,
           (pi.status IS DISTINCT FROM 'rejected')
      INTO v_pi_total, v_pi_ok
      FROM public.purchase_invoices pi
     WHERE pi.id = NEW.purchase_invoice_id;

    IF NOT COALESCE(v_pi_ok, false) THEN RETURN NEW; END IF;
    ...
    IF NOT v_pi_approved THEN RETURN NEW; END IF;
  ...
  ELSIF NEW.transaction_type = 'internal_transfer_out' THEN
    INSERT INTO public.journal_entries(entry_date, description, source_type, source_id, status)
    VALUES (NEW.expense_date, 'تحويل داخلي', 'internal_transfer', NEW.id::text, 'draft')
    RETURNING id INTO v_je;
    INSERT INTO public.journal_entry_lines(journal_entry_id, account_id, description, debit, credit, finance_account_id, line_order)
    VALUES
      (v_je, public.acct_id('internal_transfer_clearing'), 'تحويل صادر', NEW.amount, 0, NULL, 1),
      (v_je, public.acct_id('cash_bank'), 'خصم من الحساب', 0, NEW.amount, NEW.account_id, 2);
  END IF;

  -- Post entries for historical expenses (before current quarter 2026-07-01).
  -- Current-quarter entries stay as draft for manual review.
...
'ينشئ قيود مسودة للمصروفات (purchase_invoice_payment / owner_withdrawal / internal_transfer). دفع المورد لا يُنشأ قيده إلا بوجود فاتورة معتمدة ومطابقة مبلغ ضمن 0.05. القيود بتاريخ قبل 2026-07-01 تُرحَّل تلقائياً (posted)، ومن 2026-07-01 فأحدث تبقى draft للمراجعة اليدوية.';