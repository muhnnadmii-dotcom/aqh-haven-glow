...
BEGIN
  ...
  -- Supplier invoice payment: STRICTLY requires a linked, approved, amount-matched purchase invoice.
  -- Classification alone (transaction_type='supplier_invoice_payment') is NOT sufficient.
  IF NEW.purchase_invoice_id IS NOT NULL THEN
    SELECT pi.total_amount,
           (pi.status IS DISTINCT FROM 'cancelled' AND pi.deleted_at IS NULL)
      INTO v_pi_total, v_pi_ok
      FROM public.purchase_invoices pi
     WHERE pi.id = NEW.purchase_invoice_id;

    IF NOT COALESCE(v_pi_ok, false) THEN
      RETURN NEW; -- invoice missing/cancelled/deleted → leave for review
    END IF;
    ...
    IF NOT v_pi_approved THEN
      RETURN NEW; -- approval JE not posted yet → do not post payment
    END IF;

    IF ABS(COALESCE(NEW.amount,0) - COALESCE(v_pi_total,0)) > 0.05 THEN
      RETURN NEW; -- amount mismatch → leave for review
    END IF;
  ...
  ELSIF NEW.transaction_type = 'supplier_invoice_payment' THEN
    -- Classified as supplier payment but no linked purchase invoice yet → wait for linkage.
    RETURN NEW;
...
'Auto-posts finance_expenses to journal_entries. IMPORTANT: A supplier payment (Dr AP / Cr Cash) is NEVER posted based on classification alone. It requires ALL of: (1) purchase_invoice_id linked, (2) the purchase invoice exists and is not cancelled/deleted, (3) a posted purchase_invoice_approval journal entry exists for that invoice, (4) the expense amount matches the invoice total_amount within 0.05 SAR. Otherwise the row is left for manual review. Owner settlement/withdrawal and internal transfer logic unchanged. Historical entries are not auto-reversed here.';
