...
-- 0) Guard: expected counts must match before any data cleanup
DO $guard$
DECLARE
  v_notes int;
  v_refs int;
BEGIN
  SELECT count(*) INTO v_notes
   ...
   WHERE internal_notes LIKE '%تعارض مع ملف سلة بتاريخ 2026-08-17%'
     AND id IN (
       SELECT (d->>'invoice_id')::bigint
         FROM public.sales_import_batches b,
              jsonb_array_elements(b.summary_json->'details') d
        WHERE b.id = 'ac3eca33-6d98-4b11-8817-cbae618e20d7'
          AND d->>'action' = 'conflict_existing_final'
          AND d->>'invoice_id' IS NOT NULL);

  SELECT count(*) INTO v_refs
    FROM public.sales_invoices
   WHERE jsonb_typeof(payment_references) = 'array'
     AND EXISTS (SELECT 1 FROM jsonb_array_elements(payment_references) e WHERE jsonb_typeof(e) = 'string');

  IF v_notes <> 735 THEN
    RAISE EXCEPTION 'ABORT: expected 735 conflict notes for batch, found %', v_notes;
  END IF;
  IF v_refs <> 2129 THEN
    RAISE EXCEPTION 'ABORT: expected 2129 invoices with textual payment references, found %', v_refs;
  END IF;
END $guard$;

-- 1) Fix batch counters for this batch only, idempotently, from summary_json
UPDATE public.sales_import_batches b
   SET imported_rows = COALESCE((summary_json->>'new')::int,0)
                     + COALESCE((summary_json->>'updated_drafts')::int,0)
                     + COALESCE((summary_json->>'metadata_updated')::int,0),
       duplicate_rows = COALESCE((summary_json->>'unchanged')::int,0),
       needs_review_rows = COALESCE((summary_json->>'conflicts')::int,0)
                         + COALESCE((summary_json->>'needs_review')::int,0)
                         + COALESCE((summary_json->>'needs_credit_note')::int,0),
       error_rows = COALESCE((summary_json->>'blocked')::int,0)
 WHERE b.id = 'ac3eca33-6d98-4b11-8817-cbae618e20d7';

-- 2) Classifier: metadata-only differences on a DRAFT must classify as update_existing_draft
...
-- 3) Commit: conflict_existing_final must never touch data_completeness_status / internal_notes
...
BEGIN
  ...
  FOR r IN SELECT * FROM jsonb_array_elements(COALESCE(p_rows,'[]'::jsonb)) LOOP
    ...
    ELSIF act IN ('metadata_only_update','conflict_existing_final') THEN
      ...
      -- Never touches amounts, discounts, vat, invoice numbers, status, payment_status,
      -- paid_amount, settlements, journals, accounting dates, data_completeness_status
      -- or internal_notes.
  ...
  UPDATE public.sales_import_batches SET
    ...
    imported_rows = COALESCE(imported_rows,0) * (CASE WHEN v_existing_batch IS NULL THEN 0 ELSE 1 END) + n_new + n_upd + n_meta,
...
-- 4) Clean the exact conflict note text from this batch's 735 invoices only
WITH targets AS (
  SELECT DISTINCT (d->>'invoice_id')::bigint AS id
    FROM public.sales_import_batches b,
         jsonb_array_elements(b.summary_json->'details') d
   WHERE b.id = 'ac3eca33-6d98-4b11-8817-cbae618e20d7'
     AND d->>'action' = 'conflict_existing_final'
...
UPDATE public.sales_invoices s
   SET internal_notes = NULLIF(btrim(regexp_replace(
         regexp_replace(
           replace(
             replace(s.internal_notes, ' · تعارض مع ملف سلة بتاريخ 2026-08-17', ''),
             'تعارض مع ملف سلة بتاريخ 2026-08-17', ''),
           '\s*·\s*·\s*', ' · ', 'g'),
         '^\s*·\s*|\s*·\s*$', '', 'g'), ' '), '')
  FROM targets t
 WHERE s.id = t.id
   AND s.internal_notes LIKE '%تعارض مع ملف سلة بتاريخ 2026-08-17%';

-- 5) Informational backfill: textual payment references -> objects
UPDATE public.sales_invoices s
   SET payment_references = (
     SELECT COALESCE(jsonb_agg(
              CASE
                WHEN jsonb_typeof(e) <> 'string' THEN e
                WHEN btrim(e #>> '{}') LIKE '{%' THEN
                  COALESCE(
                    (SELECT CASE WHEN jsonb_typeof(x) = 'object' THEN x
                                 ELSE jsonb_build_object('reference', e #>> '{}') END
                       FROM (SELECT (e #>> '{}')::jsonb AS x) q),
                    jsonb_build_object('reference', e #>> '{}'))
                ELSE jsonb_build_object('reference', e #>> '{}')
              END
            ORDER BY ord), '[]'::jsonb)
       FROM jsonb_array_elements(s.payment_references) WITH ORDINALITY AS a(e, ord))
 WHERE jsonb_typeof(s.payment_references) = 'array'
   AND EXISTS (SELECT 1 FROM jsonb_array_elements(s.payment_references) e2 WHERE jsonb_typeof(e2) = 'string');
