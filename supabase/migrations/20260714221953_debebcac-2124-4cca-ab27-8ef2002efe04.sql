...
'Auto-post journal entries for finance_expenses. Supplier payments require: purchase_invoice_id set, invoice not rejected, posted purchase_invoice_approval entry, and amount match within 0.05 SAR. Note: purchase_invoices has no deleted_at column, so cancellation is inferred from status=rejected only.';
